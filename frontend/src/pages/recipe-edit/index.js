        ...
        onSubmit={e => {
          ...
          api
            ...
            .catch(err => {
              ...
              if (ingredients) {
                return alert(`Приорететы: ${ingredients.filter(item => Object.keys(item).length).map(item => {
                  const error = item[Object.keys(item)[0]]
                  return error && error.join(' ,')
                })[0]}`)
              }
              if (cooking_time) {
                return alert(`Сроки выполнения: ${cooking_time[0]}`)
              }
      ...
      >
        ...
        <div className={styles.cookingTime}>
          <Input
            ...
            value={recipeTime}
          />
          <div className={styles.cookingTimeUnit}>время</div>
        </div>
        <Textarea
